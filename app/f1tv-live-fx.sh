...
while $true; do
  ...
  if [[ $URL -eq "null" ]]; then
    ...
    #  #Play session with streamlink and send it with ffmpeg to a rtmp stream server
      streamlink $URL best --hls-audio-select="fx" --hls-duration 02:30:00 --record-and-pipe /dev/null | ffmpeg -hide_banner -loglevel quiet -stats -re -i pipe: -g 1 -c:v copy -c:a aac -f flv rtmp://127.0.0.1:1935/live/f1tv
    #else
	...
	#  streamlink $URL best --hls-audio-select="fx" --record-and-pipe "/record/$GLOBALNAME.mp4" | ffmpeg -re -i pipe: -c:v copy -c:a copy -f flv rtmp://127.0.0.1:1935/live/f1tv
	#fi
  fi
